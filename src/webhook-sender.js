...
class WebhookSender {
  ...
  constructor(server) {
    ...
    let queueProcessor = (job, done) => {
      ...
      server.appManager.findByKey(appKey).then((app) => {
        ...
        async
          .each(app.webhooks, (webhook, resolveWebhook) => {
            var _a;
            ...
            let pusherSignature =
              originalEventsLength !== filteredPayloadEvents.length
                ? createWebhookHmac(JSON.stringify(payload), app.secret)
                : originalPusherSignature;
            ...
            if (webhook.url) {
              axios
                .post(webhook.url, payload, { headers })
                .then((res) => {
                  if (this.server.options.debug) {
              ...
              const params = {
                ...
                InvocationType: webhook.lambda.async
                  ? "Event"
                  : "RequestResponse",
    ...
    if (server.canProcessQueues()) {
      ...
      server.queueManager.processQueue(
  ...
  sendWebhook(app, data, queueName) {
    ...
    let originalPusherSignature = createWebhookHmac(
...
module.exports = { createWebhookHmac, WebhookSender };
